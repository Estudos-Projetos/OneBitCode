type SituationPlanet = "Habitado" | "Habitável" | "Inabitável" | "Inexplorado";
...
function addPlanet(name: string, coordinates: CoordinatesPlanet, situation: SituationPlanet) {
   planets.push({
      name,
      coordinates,
      situation,
...
function rmvSatellite(name: string, planet: Planet) {
...
function findPlanetForName(name: string) {
   return planets.filter((p) => p.name === name)[0] ?? false;
}

function extractCoordinates(coordinates: string) {
...
function promptValidadePlanet(callback: (planet: Planet) => void) {
...
function showAddPlanetMenu() {
   ...
   const coordinates = extractCoordinates(
      prompt("Coordenadas separadas por virgula!\n Ex: xxxx, xxxx, xxxx, xxxx")
   );
