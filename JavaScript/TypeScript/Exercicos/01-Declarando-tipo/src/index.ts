...
function addSpaceShip(name: string, pilotName: string, crewLimit: number) {
   const spaceShip = {
      name,
      pilotName,
      crewLimit,
...
function findSpaceShip(spaceShipName: string) {
   return spaceShips.filter((s) => s.name === spaceShipName)[0];
}

function addCrew(crewName: string, spaceShip: { name: string; crew: string[]; crewLimit: number }) {
...
function sendSpaceShipInMission(spaceShip: { name: string; crew: string[]; inMission: boolean }) {
